...
class ParallelAttention(MegatronModule):
    ...
    def __init__(
        ...
        if (
            attention_type == AttnType.self_attn
            and self.attention_head_type == "multihead"
        ):
            ...
            attention_type == AttnType.self_attn
            and self.attention_head_type == "multiquery"
        ):
            ...
            self.key_value = get_linear_layer(
                args.hidden_size, 2 * args.kv_channels, init_method=init_method
            )
        elif (
            ...
            raise ValueError(
                f"Invalid attention arguments: {attention_type}, {self.attention_head_type}"
            )
        ...
        if self.attention_head_type == "multihead":
            ...
            self.core_attention = MultiQueryCoreAttention(
                self.layer_number, self.attn_mask_type
            )
        ...
        if self.use_flash_attn:
            ...
            assert (
                ...
                "supports causal and padding masks for now"
            )
            assert (
            ...
            if self.checkpoint_core_attention:
                print_rank_0(
                    "  Warning, using selective recomputation with flash-attn: this is already handled in the "
    ...
    def _checkpointed_attention_forward(
        self, query_layer, key_layer, value_layer, attention_mask, alibi
    ):
    ...
    def forward(
        ...
        if (
            self.attention_type == AttnType.self_attn
            and self.attention_head_type == "multihead"
        ):
            ...
            if get_args().sequence_parallel:
                ...
                mixed_kv_layer = mpu.gather_from_sequence_parallel_region(
                    mixed_kv_layer, tensor_parallel_output_grad=True
                )
            else:
                mixed_kv_layer = mpu.copy_to_tensor_model_parallel_region(
                    mixed_kv_layer
                )
            ...
            self.attention_type == AttnType.cross_attn
            and self.attention_head_type == "multihead"
        ):
    ...
    def __init__(
        ...
        if args.position_embedding_type == PositionEmbeddingType.alibi:
            self.alibi = self._build_alibi_tensor(
                args.seq_length, args.num_attention_heads, args.micro_batch_size
